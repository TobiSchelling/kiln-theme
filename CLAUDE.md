...
Kiln is a warm-clarity color theme designed in OKLCH color space. A single palette definition generates configs for 9 target platforms via Jinja2 templates. 4 variants: **Ember** (dark), **Forge** (dark), **Dawn** (light), **Glow** (light).
...
`palette.yaml` defines all 27 colors per variant with hex + OKLCH values, plus semantic mappings (ANSI, syntax, UI). Every color change starts here. Contains 4 variants: ember, forge, dawn, glow.
...
- Resolves `ansi_mapping`, `syntax_mapping`, `ui_mapping`, `heading_mapping` from color names to actual values
- Renders per-variant targets to `dist/{target}/{variant}/` and combined targets (obsidian) to `dist/{target}/`

### Templates (`templates/`)

Jinja2 templates receive a context with: `colors` (all 27 by name), `base_colors`/`text_colors`/`accent_colors` (pre-filtered), `ansi`/`syntax`/`ui`/`headings` (resolved mappings), plus metadata (`variant`, `display_name`, `version`).
...
- **Combined targets** (`COMBINED_TARGETS`): `obsidian/` renders ember (dark) + glow (light) into one file using `dark.*`/`light.*` access
...
- **OKLCH-first:** Accents at uniform perceptual lightness (~0.65 dark, ~0.52 light for Glow, ~0.43 for Dawn)
- **WCAG AA (4.5:1):** All text/accent colors except `comment` (intentionally low-contrast)
- **Signature accent:** Peach — used for properties, parameters, inline code
